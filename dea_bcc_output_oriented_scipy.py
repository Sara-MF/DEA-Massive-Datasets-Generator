...
def dea_bcc_output_scipy(csv_path):
    ...
    for i in range(n):
        ...
        # Since the BCC output-oriented DEA model maximizes phi, the problem is converted to min(-phi).
        # [0]*n creates 0 coefficients for the lambdasand [-1] adds the coefficient for phi.
        ...
        # Σ(λ_j) = 1
        # The first n coefficients correspond to the lambdas and the last coefficient corresponds to phi.
        # λ1 + λ2 + ... + λn + 0*φ = 1
        equality_matrix = [
            [1] * n + [0]
        ]
